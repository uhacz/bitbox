...
float sampleShadowMap_optimizedPCF( in float3 shadowPos, in float3 shadowPosDX, in float3 shadowPosDY, in uint cascadeIdx )
{
    ...
    const float bias = (1.f + pow( 2.f, (float)cascadeIdx )) / shadowMapSize.y;
    ...
    lightDepth -= max( fractionalSamplingError, 0.01 );
...
float ps_shadow( in in_PS_shadow input ) : SV_Target0
{
    ...
    float3 shadowPosDX = ddx_fine( shadowPos.xyz );
    float3 shadowPosDY = ddy_fine( shadowPos.xyz );
