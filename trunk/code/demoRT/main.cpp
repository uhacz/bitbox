...
Sphere spheres[] = {//Scene: radius, position, emission, color, material
    ...
    Sphere(16.5f, Vec(27.f,16.5f,47.f),       Vec(),Vec(1,1,1)*.999f, SPEC),//Mirr
    Sphere(16.5f, Vec(73.f,16.5f,78.f),       Vec(),Vec(1,1,1)*.999f, REFR),//Glas
...
inline Vec radiance(const Ray &r) {
    ...
    for (;; ++depth)
    {
        if (depth>5 || !intersect(currentRay, t, id)) {
...
int main(int argc, char *argv[]){
    int w=640;
    int h=480;
    int samps = 64; //argc==2 ? atoi(argv[1]) : 1; // # samples
