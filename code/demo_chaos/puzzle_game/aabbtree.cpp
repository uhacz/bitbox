...
void AABBTree::CalculateFaceBounds(uint32_t* faces, uint32_t numFaces, Vector3F& outMinExtents, Vector3F& outMaxExtents)
{
    ...
    for (uint32_t i=0; i < numFaces; ++i)
    {
        ...
        minExtents = minPerElem(a, minExtents);
        maxExtents = maxPerElem(a, maxExtents);

        minExtents = minPerElem(b, minExtents);
        maxExtents = maxPerElem(b, maxExtents);

        minExtents = minPerElem(c, minExtents);
        maxExtents = maxPerElem(c, maxExtents);
    }
...
uint32_t AABBTree::PartitionSAH(Node& n, uint32_t* faces, uint32_t numFaces)
{
    ...
    for (uint32_t i=0; i < numFaces; ++i)
    {
        Vector3F v = 0.5f*(m_faceBounds[faces[i]].m_min + m_faceBounds[faces[i]].m_max) - mean;
        v *= v;
        variance += v;
    }
...
/*
bool AABBTree::TraceRay(const Vector3F& start, const Vector3F& dir, float& outT, float& outU, float& outV, float& outW, float& outSign, uint32_t& outIndex) const
{
#if _WIN32
    // reset stats
    s_traceDepth = 0;
    ...
    const Vector3F rcp_dir(1.0f/dir.x, 1.0f/dir.y, 1.0f/dir.z);
...
{   
    ...
    Vector3F rcp_dir(1.0f/dir.x, 1.0f/dir.y, 1.0f/dir.z);
...
/*
bool AABBTree::TraceRay(const Vector3F& start, const Vector3F& dir, float& outT, Vector3F* outNormal) const
{   
    outT = FLT_MAX;
...
void AABBTree::TraceRecursive(uint32_t n, const Vector3F& start, const Vector3F& dir, float& outT, Vector3F* outNormal) const
{
...
void AABBTree::DebugDrawRecursive(uint32_t nodeIndex, uint32_t depth)
{
    ...
    n.m_center = Vector3F(minExtents+maxExtents)/2;
    n.m_extents = (maxExtents-minExtents)/2;
    */
/*
    if (n.m_minEtextents != Vector3F(0.0f))
    {
        Vector3F corners[8];
        corners[0] = n.m_center + Vector3F(-n.m_extents.x, n.m_extents.y, n.m_extents.z);
        corners[1] = n.m_center + Vector3F(n.m_extents.x, n.m_extents.y, n.m_extents.z);
        corners[2] = n.m_center + Vector3F(n.m_extents.x, -n.m_extents.y, n.m_extents.z);
        corners[3] = n.m_center + Vector3F(-n.m_extents.x, -n.m_extents.y, n.m_extents.z);

        corners[4] = n.m_center + Vector3F(-n.m_extents.x, n.m_extents.y, -n.m_extents.z);
        corners[5] = n.m_center + Vector3F(n.m_extents.x, n.m_extents.y, -n.m_extents.z);
        corners[6] = n.m_center + Vector3F(n.m_extents.x, -n.m_extents.y, -n.m_extents.z);
        corners[7] = n.m_center + Vector3F(-n.m_extents.x, -n.m_extents.y, -n.m_extents.z);
