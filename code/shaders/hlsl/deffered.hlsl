...
in_PS vs_geometry_main(in_VS IN)
{
    ...
    OUT.hpos = mul( _viewProj, float4( wpos, 1.0 ) );
    OUT.vpos = mul( _view, wpos ).xyz;
...
out_PS ps_geometry_main( in_PS IN )
{
    ...
    specular_value *= specular_tex.Sample( _sampl_point, uv ).r;
    roughness_value *= roughness_tex.Sample( _sampl_point, uv ).r;
    metallic_value *= metallic_tex.Sample( _sampl_point, uv ).r;
#endif
