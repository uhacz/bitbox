...
namespace motion_fields
{
    ...
    //    const Vector3 root_displacement_y = mulPerElem( joints[0].position, Vector3( 0.f, 1.f, 0.f ) );
    ...
    //    params.velocity = v1;
    //    params.acceleration = (v1 - v0) * clip->sampleFrequency;
    //    
    //    //joints_scratch[0].position = mulPerElem( joints_scratch[0].position, Vector3( 0.f, 1.f, 0.f ) );
    //}

    void MotionMatching::posePrepare( Pose* pose, const PosePrepareInfo& info )
    {
        ...
        bxAnim_Joint trajectory_joint;
        const float duration = 1.f; // clip->duration;
        const float dt = ( duration ) / (float)( eNUM_TRAJECTORY_POINTS - 1 );
        const float start_time = 0.f; // (float)frameNo / clip->sampleFrequency;
        ...
        for( u32 i = 0; i < eNUM_TRAJECTORY_POINTS; ++i )
        {
            ...
            else
            {
                ...
                if( i == 0 )
                {
                    pose->trajectory[i] = last_joint.position;
                }
                else
                {
                    const floatInVec displacement = length( trajectory_joint.position );

                    pose->trajectory[i] = pose->trajectory[i - 1] + normalizeSafe( v ) * displacement;
                }
            }
...
//-----------------------------------------------------------------------
    ...
    void MotionMatching::prepare()
    {
        ...
        //    "RightToeBase",
        //};
        const u32 num_match_joints_names = sizeof( match_joints_names ) / sizeof( *match_joints_names );

        _data.match_joints_indices.resize( num_match_joints_names, -1 );

        for( u32 i = 0; i < num_match_joints_names; ++i )
        {
            _data.match_joints_indices[i] = bxAnim::getJointByName( _data.skel, match_joints_names[i] );
        }
    ...
    float _ComputePoseCost( const bxAnim_Joint* aJoints, const bxAnim_Joint* bJoints, const float* betha, u32 numJoints, const i16* jointIndices, u32 numJointIndices )
    {
        ...
        const Vector3 u = normalize( Vector3( 0.23742528f, -0.546734f, 0.7695485349f ) );

        const float vr = lengthSqr( aJoints[0].position - bJoints[0].position ).getAsFloat() * betha[0];
        const float q0 = lengthSqr( rotate( aJoints[0].rotation, u ) - rotate( bJoints[0].rotation, u ) ).getAsFloat() * betha[0];

        //for( u32 i = 0; i < numJoints; ++i )
        for( u32 i = 1; i < numJointIndices; ++i )
        {
            const i16 index = jointIndices[i];
            //const Vector3 a = aJoints[index].position;
            //const Vector3 b = bJoints[index].position;
            //result += betha[index] * lengthSqr( a - b ).getAsFloat();

            const Quat qa = aJoints[index].rotation;
            const Quat qb = bJoints[index].rotation;
            const Vector3 qav = rotate( qa, u );
            const Vector3 qbv = rotate( qb, u );
            result += betha[index] * lengthSqr( qav - qbv ).getAsFloat();
            //const float d = 1.f - dot( qa, qb ).getAsFloat();
            //result += d;
        }

        result = ::sqrt( vr + q0 + result );
        //const float denom = 1.f; // / ( numJointIndices );
        return ( result );// *denom;
    }

    float _ComputePositionCost( const Vector3* pos0, const Vector3* pos1, u32 numPos )
    {
        floatInVec result( 0 );
    ...
    float _ComputeDirectionCost( const Vector3* dir0, const Vector3* dir1, u32 n )
    {
    ...
    float _ComputeVelocityCost( const Vector3* vel0, const Vector3* vel1, u32 numPos )
    {
        floatInVec result( 0 );
        for( u32 i = 0; i < numPos; ++i )
        {
    ...
    inline void _BuildTrajectoryCurve( Curve3D* c, const Vector3* trajectory )
    {
        const float delta = 1.f / (float)( eNUM_TRAJECTORY_POINTS - 1 );
        float t = 0.f;
        curve::clear( *c );
        for( u32 i = 0; i < eNUM_TRAJECTORY_POINTS; ++i )
        {
            curve::push_back( *c, trajectory[i], t );
            t += delta;
        }
    }
    float _ComputeTrajectoryCost1( const Curve3D& c0, const Curve3D& c1 )
    {
        float cost = 0.f;
        const float delta = 1.f / (float)( eNUM_TRAJECTORY_POINTS - 1 );
        float t = 0.f;
        for( u32 i = 0; i < eNUM_TRAJECTORY_POINTS; ++i )
        {
            Vector3 p0 = curve::evaluate_catmullrom( c0, t );
            Vector3 p1 = curve::evaluate_catmullrom( c1, t );

            cost += lengthSqr( p1 - p0 ).getAsFloat();
            t += delta;
        }
    ...
    void _ComputeTrajectoryDerivatives( Vector3 v[eNUM_TRAJECTORY_POINTS-1], Vector3 a[eNUM_TRAJECTORY_POINTS-2], const Vector3 x[eNUM_TRAJECTORY_POINTS], float deltaTimeInv )
    {
    ...
    void MotionMatching::tick( const Input& input, float deltaTime )
    {
        ...
        _ComputeTrajectoryDerivatives( local_trajectory_v, local_trajectory_a, local_trajectory, delta_time_inv );
        ...
        if( _state.last_match_time_s >= 0.016f )
        {
            ...
            if( curr_local_joints && _state.num_clips == 1)
            {
                ...
                for( size_t i = 0; i < _data.poses.size(); ++i )
                {
                    ...
                    Vector3 pose_trajectory_v[eNUM_TRAJECTORY_POINTS - 1];
                    Vector3 pose_trajectory_a[eNUM_TRAJECTORY_POINTS - 2];
                    _ComputeTrajectoryDerivatives( pose_trajectory_v, pose_trajectory_a, pose.trajectory, clip->sampleFrequency );

                    const float trajectory_position_cost = _ComputePositionCost( local_trajectory, pose.trajectory, eNUM_TRAJECTORY_POINTS );
                    float trajectory_direction_cost = _ComputeDirectionCost( local_trajectory_v, pose_trajectory_v, eNUM_TRAJECTORY_POINTS - 1 );
                    trajectory_direction_cost += _ComputeDirectionCost( local_trajectory_a, pose_trajectory_a, eNUM_TRAJECTORY_POINTS - 2 );
                    ...
                    const float trajectory_velocity_cost     = _ComputeVelocityCost( local_trajectory_v, pose_trajectory_v, eNUM_TRAJECTORY_POINTS - 1 ) * match_delta_time;
                    const float trajectory_acceleration_cost = _ComputeVelocityCost( local_trajectory_a, pose_trajectory_a, eNUM_TRAJECTORY_POINTS - 2 ) * match_delta_time;
        ...
        {
            _state.last_match_time_s += deltaTime;
        }
        if( curr_local_joints )
        {
            ...
            //for( auto pose_index : _debug.pose_indices )
            //{
            //    u32 color = ( pose_index == _state.pose_index ) ? 0xFF0000FF : 0x666666FF;
            //    _DebugDrawPose( pose_index, color, input.base_matrix );
            //}
