...
namespace bxGame
{
    ...
    struct Character1
    {
        ...
        Constraint constraints[eCONSTRAINT_COUNT];
    ...
    Character1* character1_new()
    {
        ...
        ch->_dtAcc = 0.f;
        ch->_jumpAcc = 0.f;
        memset( &ch->particles, 0x00, sizeof( Character1::Particles ) );
        memset( &ch->constraints, 0x00, sizeof( bxGame::Constraint ) );
    ...
    {
        Character1::Particles& p = character->particles;
        ...
        for( int i = 0; i < Character1::ePARTICLE_COUNT; ++i )
        {
            p.pos1[i] = p.pos0[i];
            p.vel[i] = Vector3( 0.f );
        ...
        {
            Constraint& c0 = character->constraints[0];
            c0.i0 = 0;
            c0.i1 = 1;
            c0.d = length( p.pos0[0] - p.pos0[1] ).getAsFloat();

            Constraint& c1 = character->constraints[1];
            c1.i0 = 0;
            c1.i1 = 2;
            c1.d = length( p.pos0[0] - p.pos0[2] ).getAsFloat();

            Constraint& c2 = character->constraints[2];
            c2.i0 = 1;
            c2.i1 = 2;
            c2.d = length( p.pos0[1] - p.pos0[2] ).getAsFloat();
        }

        character->contacts = bxPhx::contacts_new( Character1::ePARTICLE_COUNT );
    ...
    void character1_collectInputData( Character1::Input* charInput, const bxInput& input, float deltaTime )
    {
    ...
    {
        character1_collectInputData( &character->input, input, deltaTime );
        Vector3 externalForces( 0.f );
        {
            ...
            //externalForces += character->upVector * character->input.jump * 10.f;
        ...
        {
            Character1::Particles* cp = &character->particles;
            ...
            const int nConstraint = Character1::eCONSTRAINT_COUNT;
...

            {
                ...
                bxPhx::collisionSpace_collide( cspace, character->contacts, character->particles.pos1, nPoint );
            }
    ...
    {
        return Matrix4( Matrix3::identity(), character->particles.pos0[0] );
    }
